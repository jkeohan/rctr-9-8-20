...
As the world of web development and software engineering grows in complexity so does the need to leverage tools that facilitate development.  
...
- Think about what a front end framekwork is, why we use it and how it's different than a JavaScript library like jQuery, Lodash, Underscore or Ramda. 
...
A framework is software providing generic functionality and structure that serves as foundation to build and deploy applications.  The following are just a few of the front end frameworks mentioned in [https://2018.stateofjs.com](https://2018.stateofjs.com/front-end-frameworks/overview/)
...
**Question:** Have you worked in any front end framework before?
...
First, let's think about where you might see a React.js app. Here are two of the most populare web sites built in React:
...
Let's take a look at this article and slack out any  [32 Web Sites Built In React](https://medium.com/@coderacademy/32-sites-built-with-reactjs-172e3a4bed81)

- Look over the sites and determine which ones you use on a daily or weekly basis
- When asked slack your answer in the thread created by the instructor
...
So a few things have been removed and need to be configured on our part to get our React app up and running properly.  For this demo we will focus on:

- Add the required libraries (react & reactDOM)
- Import the libraries into inxdex.js
...
#### ReactDOM.render()
With our libraries in place we can use ReactDOM.render() to render Componnents, in our basic starter app some basic `JSX` for now.  Although the `<h`>`
...
`ReactDOM.render` takes the Virtual DOM node  and
...
**Question:** We know that `Hello World` is being rendered from `index.js` but where is the rest of the HTML coming from?
